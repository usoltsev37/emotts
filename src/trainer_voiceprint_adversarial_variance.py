...
from src.data_process.voiceprint_variance_adaptor_dataset import (
    VoicePrintVarianceBatch,
...
class Trainer:
    ...
    def __init__(self, config: TrainParams):
        ...
        self.feature_model = NonAttentiveTacotronVoicePrintVarianceAdaptor(
            ...
            variance_adaptor_config=config.variance_adapter_params
        ).to(self.device)
        ...
        self.criterion = NonAttentiveTacotronVarianceAdaptorLoss(
    ...
    def batch_to_device(self, batch: VoicePrintVarianceBatch) -> VoicePrintVarianceBatch:
        batch_on_device = VoicePrintVarianceBatch(
    ...
    def prepare_loaders(
        self,
    ) -> Tuple[DataLoader[VoicePrintVarianceBatch], DataLoader[VoicePrintVarianceBatch]]:

        factory = VoicePrintVarianceFactory(
            sample_rate=self.config.sample_rate,
            hop_size=self.config.hop_size,
        ...
        collate_fn = VoicePrintVarianceCollate()
    ...
    def calc_adv_loss(
        self, style_emb: torch.Tensor, batch: VoicePrintVarianceBatch
    ...
    def train(self) -> None:
        ...
        while self.iteration_step < self.config.total_iterations:
            for batch in self.train_loader:
                ...
                loss_prenet, loss_postnet, loss_durations, loss_pitch, loss_energy = self.criterion(
                    mel_outputs,
                    mel_outputs_postnet,
                    durations,
                    batch.durations,
                ...
                loss = loss_prenet + loss_postnet + loss_durations + loss_pitch + loss_energy
    ...
    def validate(self) -> None:
        with torch.no_grad():
            ...
            for batch in self.valid_loader:
                batch = self.batch_to_device(batch)
                durations, mel_outputs_postnet, mel_outputs, _, pitch, energy = self.feature_model(
                    batch
                )
                loss_prenet, loss_postnet, loss_durations, loss_pitch, loss_energy = self.criterion(
                    mel_outputs,
            ...
            val_loss_pitch = val_loss_pitch / len(self.valid_loader)
            val_loss_energy = val_loss_energy / len(self.valid_loader)
            self.write_losses(
                "valid",
                {
                    "total": val_loss,
                    ...
                    "pitch": val_loss_pitch,
                    "energy": val_loss_energy,
